#!/usr/bin/env python3
"""
Minimal API Test - Test the complete flow with real OpenAI API calls using minimal dependencies
"""
...
async def test_minimal_flow():
    """Test the minimal flow with real API calls."""
    print("🚀 MINIMAL API TEST")
    print("=" * 50)
    
    start_time = time.time()
    
    # Step 1: Load sample data from prompt_injection.json
    print("\n📊 Step 1: Loading sample data from prompt_injection.json...")
    
    with open('data/prompt_injection.json', 'r', encoding='utf-8') as f:
        dataset = json.load(f)
    
    # Take only first 2 samples for testing
    ...
    try:
        # Try to import and use OpenAI client
        ...
        # Test basic API call
        print(f"   Testing basic API call to {TARGET_MODEL}...")
    ...
    # Step 3: Capture responses from GPT-3.5-turbo
    print(f"\n⚡ Step 3: Capturing responses from GPT-3.5-turbo...")
    ...
    # Step 4: Evaluate captured responses using GPT-4.1-mini
    print(f"\n🧠 Step 4: Evaluating responses using GPT-4.1-mini...")
...
if __name__ == "__main__":
    success = asyncio.run(test_minimal_flow())
    if success:
