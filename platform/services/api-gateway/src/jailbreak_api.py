#!/usr/bin/env python3
"""
Jailbreak API Endpoint for Frontend Integration
"""
...
@router.post("/test/jailbreak/start", response_model=JailbreakTestResponse)
async def start_jailbreak_test(request: JailbreakTestRequest, background_tasks: BackgroundTasks):
    """Start a jailbreak test run."""
    try:
        # Generate unique test ID
        test_id = f"jailbreak_{int(time.time())}_{str(uuid.uuid4())[:8]}"
...
@router.get("/test/jailbreak/{test_id}/status", response_model=JailbreakTestResponse)
...
@router.get("/test/jailbreak/{test_id}/results")
...
@router.get("/test/jailbreak/{test_id}/download")
...
async def execute_jailbreak_test(test_id: str, request: JailbreakTestRequest):
    ...
    try:
        ...
        limited_samples = dataset[:MAX_PROMPTS]
