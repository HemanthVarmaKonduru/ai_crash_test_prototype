#!/usr/bin/env python3
"""
Prompt Injection API Endpoint for Frontend Integration
"""
...
MAX_PROMPTS = int(os.getenv("MAX_PROMPTS_PI", "30"))  # Limit to first 30 prompts
...
@router.post("/test/prompt-injection/start", response_model=TestResponse)
async def start_prompt_injection_test(request: TestRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        test_id = f"test_{int(time.time())}_{str(uuid.uuid4())[:8]}"
        ...
        return TestResponse(
            ...
            progress=0,
            current_step="Loading dataset...",
            total_tests=MAX_PROMPTS
        )
        
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to start test: {str(e)}")

@router.get("/test/prompt-injection/{test_id}/status", response_model=TestResponse)
...
@router.get("/test/prompt-injection/{test_id}/results")
async def get_test_results(test_id: str):
    """Get the final results of a completed test."""
    if test_id not in test_sessions:
        raise HTTPException(status_code=404, detail="Test not found")
...
@router.get("/test/prompt-injection/{test_id}/download")
async def download_test_report(test_id: str):
    """Download the test report as JSON."""
    if test_id not in test_sessions:
        raise HTTPException(status_code=404, detail="Test not found")
...
async def execute_prompt_injection_test(test_id: str, request: TestRequest):
    ...
    try:
        ...
        session["current_step"] = "Loading dataset..."
        session["progress"] = 5
